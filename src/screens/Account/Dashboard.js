      ...
      .once('value', snapshot => {
        if (snapshot.exists()) {
          ...
          console.log(
            ...
            payments.length,
            'members',
            chosenGroup.members_list.length,
          );
          if (payments.length >= chosenGroup.members_list.length) {
            toast.show(
              `Payment of £${amount} to ${transactionData.groupName} for contribution payment was unsuccessful because you have completed payments!`,
              {
                type: 'warning',
                placement: 'bottom',
                duration: 5000,
              ...
              .once('value', snapshot => {
                ...
                if (group) {
                  ...
                  reference.ref(`groups/${selectedGroup[0].id}`).update({
                    wallet_balance:
                      Number(selectedGroup[0].wallet_balance) + Number(amount),
                  });
                  const groupChildRef = database()
                    .ref('groups')
                    .child(selectedGroup[0].id);
                  groupChildRef.once('value', snapshot => {
                    // console.log('snapshot',snapshot);
                    if (snapshot.val().id === selectedGroup[0].id) {
                      groupChildRef.update({
                        wallet_balance:
                          Number(selectedGroup[0].wallet_balance) +
                          Number(amount),
                      });
                    }
                  });
                  toast.show(
                    `Payment of £${amount} to ${transactionData.groupName} 
      ...
      <ScrollView style={{width: '100%'}}>
        ...
        {availableGroups.length > 0 ? (
          <View style={styles.makeDepositView}>
            <View style={{width: '100%'}}>
              <Text
                style={{color: colors.green, fontWeight: 'bold', fontSize: 25}}>
                Quick Transfer
              </Text>
            </View>
          ...
          <View style={styles.makeDepositView}>
            <Text
              ...
              You have not been added to a group.
            </Text>
          </View>
        )}
      </ScrollView>
